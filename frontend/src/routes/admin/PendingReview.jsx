              ...
              {reviews.map(r => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* r.title is DB content — translated dynamically */}
                    ...
                    {r.confidence && (
                      <p className={`text-sm font-semibold mt-2 ${r.confidence >= 0.8 ? 'text-[#1D9E75]' : 'text-[#BA7517]'}`}>
                        {tx('OCR Confidence')}: {Math.round(r.confidence * 100)}%
                      </p>
                    )}
                  </div>
                );
          ...
          <div className="hidden lg:block w-96 bg-white rounded-2xl shadow-lg border border-[#D3D1C7] overflow-hidden fixed right-8 top-24 h-[calc(100vh-200px)]">
            <div className="p-6 h-full flex flex-col overflow-y-auto">
              ...
              <div className="space-y-4 flex-1">
                ...
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-xs text-[#5F5E5A] mb-1">{tx('Village', 'village')}</p>
                  <p className="font-semibold">{selectedReview.village}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-xs text-[#5F5E5A] mb-1">{tx('Type')}</p>
                  <p className="font-semibold">
                    {tx(typeBadge[selectedReview.type]?.label || selectedReview.type)}
                  </p>
                </div>

                {selectedReview.confidence && (
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-xs text-[#5F5E5A] mb-1">{tx('OCR Confidence')}</p>
